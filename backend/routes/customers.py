...
async def import_customers_from_cfdis(request: Request, current_user: Dict = Depends(get_current_user)):
    """
    Importa clientes desde CFDIs de tipo 'ingreso'.
    Fuente: receptor_nombre + receptor_rfc de db.cfdis.
    ...
    cfdis = await db.cfdis.find(
        {"company_id": company_id, "tipo_cfdi": "ingreso"},
        {"receptor_nombre": 1, "receptor_rfc": 1, "_id": 0},
    ).to_list(5000)
    ...
    for cfdi in cfdis:
        nombre = (cfdi.get("receptor_nombre") or "").strip()
        rfc    = (cfdi.get("receptor_rfc") or "").strip().upper() or None
        if not nombre:
            continue
        key = rfc or nombre
        if key in seen:
    ...
    logger.info(f"[IMPORT CUSTOMERS] company={company_id} importados={importados} existentes={existentes}")
