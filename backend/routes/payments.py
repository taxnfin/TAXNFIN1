...
async def get_payments_summary(request: Request, current_user: Dict = Depends(get_current_user)):
    """Get payments summary with totals by currency"""
    company_id = await get_active_company_id(request, current_user)
    ...
    pagos_pendientes = 0
    cobros_pendientes = 0
    domiciliaciones_activas = 0
    
    for p in payments:
        moneda = p.get('moneda', 'MXN')
        monto = p.get('monto', 0)
        tipo = p.get('tipo', 'pago')
        estatus = p.get('estatus', 'pendiente')
        
        if estatus == 'pendiente':
            if tipo == 'pago':
                if moneda == 'USD':
                    total_por_pagar_usd += monto
                else:
                    total_por_pagar += monto
                pagos_pendientes += 1
            else:  # cobro
                if moneda == 'USD':
                    total_por_cobrar_usd += monto
                else:
                    total_por_cobrar_mxn += monto
                cobros_pendientes += 1
        
        if p.get('domiciliacion_activa'):
            monto_domiciliado += monto
            domiciliaciones_activas += 1
        
        # Check if paid this month
        fecha_pago_str = p.get('fecha_pago')
        if fecha_pago_str and estatus == 'completado':
            try:
                fecha_pago = datetime.fromisoformat(fecha_pago_str) if isinstance(fecha_pago_str, str) else fecha_pago_str
                if month_start <= fecha_pago < next_month:
                    if tipo == 'pago':
                        if moneda == 'USD':
    ...
    return {
        ...
        'total_por_pagar_usd': total_por_pagar_usd,
        'total_por_cobrar': total_por_cobrar_mxn,
        'total_por_cobrar_mxn': total_por_cobrar_mxn,
        'total_por_cobrar_usd': total_por_cobrar_usd,
        'monto_domiciliado': monto_domiciliado,
        'total_pagado_mes': pagado_mes_mxn + pagado_mes_usd,
        'pagado_mes_mxn': pagado_mes_mxn,
        'pagado_mes_usd': pagado_mes_usd,
        'total_cobrado_mes': cobrado_mes_mxn + cobrado_mes_usd,
        'cobrado_mes_mxn': cobrado_mes_mxn,
        'cobrado_mes_usd': cobrado_mes_usd,
        'pagos_pendientes': pagos_pendientes,
        'cobros_pendientes': cobros_pendientes,
        'domiciliaciones_activas': domiciliaciones_activas,
        'fecha_corte': (today + timedelta(days=30)).isoformat()
    }
