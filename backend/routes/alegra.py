...
@router.get("/recalcular-bills")
async def recalcular_bills_get(
    request: Request,
    company_id: str = Query(..., description="Company ID (prefix ok)"),
    current_user: Dict = Depends(get_current_user),
):
    """GET version de recalcular-bills para uso desde el navegador (solo hola@taxnfin.com)."""
    if current_user.get('email') != 'hola@taxnfin.com':
        raise HTTPException(status_code=403, detail="Acceso restringido a administrador del sistema")
