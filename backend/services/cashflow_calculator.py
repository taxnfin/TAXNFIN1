"""Lee cashflow_weeks de MongoDB y expone los datos para el Calendario de Tesorería."""
...
async def calcular_semanas_cashflow(company_id: str, num_weeks: int = 52) -> List[Dict]:
    """
    Lee cashflow_weeks de MongoDB — los mismos datos que muestra el Cash Flow.
    Agrega semanas generadas dinámicamente si hay menos de num_weeks.
    """
    ...
    for week in weeks_raw:
        ...
        if week_id:
            cfdis = await db.cfdis.find(
                {'company_id': company_id, 'cashflow_week_id': week_id},
        ...
        for c in cfdis:
            ...
            item = {
                ...
                'concepto': nombre,
                'monto': monto,
                'fecha': str(c.get('fecha_emision', ''))[:10],
        ...
        # Usar totales de cashflow_weeks si no hay CFDIs con week_id
        ...
        result.append({
            ...
            'total_ingresos_reales': float(week.get('total_ingresos_reales', total_ing) or total_ing),
            'total_egresos_reales': float(week.get('total_egresos_reales', total_egr) or total_egr),
    ...
    if len(result) < num_weeks and result:
        ...
        for i in range(num_weeks - len(result)):
            ...
            result.append({
                ...
                'numero_semana': num,
                'label': f'S{num}',
                'fecha_inicio': ws.isoformat(),
                'fecha_fin': we.isoformat(),
